...
def struc_unpack(exp, data, pos):
    try:
        return struct.unpack(exp, data)[pos]
    except:
        return None
...
class lnk_file:
    ...
    def parse_lnk_header(self):
        ...
        try:
            ...
            self.lnk_header['guid'] = codecs.encode(lnk_header[4:20], 'hex')
            ...
            self.lnk_header['rfile_size'] = codecs.encode(lnk_header[52:56], 'hex')

            self.lnk_header['icon_index'] = struct.unpack('<I', lnk_header[56:60])[0]
            try:
                if False: #struct.unpack('<i', lnk_header[60:64])[0] < (self.WINDOWSTYLES):
            ...
            try:
                self.lnk_header['hotkey'] = "%s - %s {0x%s}" % (
                    ...
                    codecs.encode(lnk_header[64:66], 'hex')
                )
                ...
                #raise e
                    ...
                    #print(codecs.encode(lnk_header[65:66], 'hex'))
        ...
        #print("header_size=%s" % repr(self.lnk_header["header_size"]))
    ...
    def parse_link_information(self):
        ...
        while True:
            ...
            tmp_item['rsize'] = codecs.encode(self.link_target_list[index : index + 2], 'hex')
...
#			print self.indata[index + 2: index + 2 + ItemID['size']].replace('\x00','')
            ...
            try:
                ...
                if self.loc_information['locationFlags'] & 0x0001:
                    ...
                    if self.loc_information['VolumeIDAndLocalBasePath']['o_VolLabel'] > 16:
                        tmp = struct.unpack('<i', self.indata[index + 16 : index + 20])[0]
                        self.loc_information['VolumeIDAndLocalBasePath']['o_VolLabel'] = vo

                    if self.loc_information['VolumeIDAndLocalBasePath']['rdriveType'] < len(self.DRIVE_TYPES):
                        tmp = self.DRIVE_TYPES[self.loc_information['VolumeIDAndLocalBasePath']['rdriveType']]
                        self.loc_information['VolumeIDAndLocalBasePath']['driveType'] = tmp
                    ...
                    tmp = {
                        ...
                        "sizeNetInformation": struc_unpack('<i', self.indata[index+0:index+4], 0),
                        #"netShareFlags" : struct.unpack('<i', self.indata[index + 4 : index + 8])[0],
                        "netShareFlags": struc_unpack('<i', self.indata[index+4:index+8], 0),
                        #"o_netShareName" : struct.unpack('<i', self.indata[index + 8 : index + 12])[0],
                        "o_netShareName": struc_unpack('<i', self.indata[index+8:index+12], 0),
                        #"o_netDeviceName" : struct.unpack('<i', self.indata[index + 12 : index + 16])[0],
                        "o_netDeviceName": struc_unpack('<i', self.indata[index+12:index+16], 0),
                        #"netProviderType" : struct.unpack('<i', self.indata[index + 16 : index + 20])[0],
                        "netProviderType": struc_unpack('<i', self.indata[index+16:index+20], 0),
                    }
                    self.loc_information['CommonNetworkRelativeLinkAndPathSuffix'] = tmp

                    try:
                        if self.loc_information['VolumeIDAndLocalBasePath']['o_VolLabel'] > 16:
                            tmp = struct.unpack('<i', self.indata[index + 16 : index + 20])[0]
                            self.loc_information['VolumeIDAndLocalBasePath']['o_VolLabel'] = tmp
    ...
    def parse_distributedTracker_block(self, index, size):
        tmp = {}
        tmp['size'] = struct.unpack('<I', self.indata[index + 8: index + 12])[0]
        tmp['version'] = struct.unpack('<I', self.indata[index + 12: index + 16])[0]
        tmp['machine_identifier'] = self.clean_line(self.indata[index + 16: index + 32])
        tmp['droid_volume_identifier'] = codecs.encode(self.indata[index + 32: index + 48],'hex')
        tmp['droid_file_identifier'] = codecs.encode(self.indata[index + 48: index + 64], 'hex')
        tmp['birth_droid_volume_identifier'] = codecs.encode(self.indata[index + 64: index + 80 ], 'hex')
        tmp['birth_droid_file_identifier'] = codecs.encode(self.indata[index + 80: index + 96 ], 'hex')
        self.extraBlocks['DISTRIBUTED_LINK_TRACKER_BLOCK'] = tmp
    ...
    if len(sys.argv) > 1:
        for arg in sys.argv:
            #with open(arg, "rb") as fh:
                #lnk = lnk_file(fhandle=fh, debug=True)
                #lnk.print_lnk_file()
            fh = io.open(arg, "rb")
            lnk = lnk_file(fhandle=fh, debug=True)
            lnk.print_lnk_file()
